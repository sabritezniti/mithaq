...
with st.sidebar:
    ...
    if logged_in():
        ...
        <div style='background:#1e3a5f;padding:10px 14px;border-radius:10px;margin-bottom:16px'>
          ...
          <p style='margin:3px 0 0;font-size:10px;color:#475569'>🔑 {u['access_key']}</p>
        </div>""", unsafe_allow_html=True)
...
# ── PAYMENT ───────────────────────────────────────────────────────────────────
    ...
    if u.get("is_paid"):
        ...
        st.markdown(f'<div class="card"><h4 style="color:#22c55e">عضويتك نشطة ✓</h4>'
                    f'<p>مفتاح الوصول: <code>{u["access_key"]}</code></p></div>',
                    unsafe_allow_html=True)
    else:
        c1,c2,c3 = st.columns(3)
        plans = [
